...
<div class="uo-page">
    ...
    <div class="uo-cov">
        ...
        <div class="uo-bar">
        <?php foreach ([
            ['seg-full',   $n_full, _('Fully configured')],
            ['seg-media',  $n_mo,   _('Media only')],
            ['seg-action', $n_ao,   _('Action only')],
            ['seg-none',   $n_none, _('Not configured')],
        ] as [$cls, $n, $tip]):
            if ($n <= 0) continue;
        ?>
    ...
    <?php foreach ($users as $user):
        ...
        $has_med  = !empty($medias);
        $has_act  = !empty($actions);
        $conf_cls = ($has_med && $has_act) ? 'conf-full' : (($has_med || $has_act) ? 'conf-half' : 'conf-none');
    ...
    ?>
    <div class="uo-card <?= $conf_cls ?>" data-search="<?= htmlspecialchars($search) ?>">
        ...
        <div class="uo-card-head">
            ...
            <div class="uo-score-dots">
                <div class="uo-sdot <?= $has_med ? 'on-med' : '' ?>" title="<?= _('Media') ?>"></div>
                <div class="uo-sdot <?= $has_act ? 'on-act' : '' ?>" title="<?= _('Actions') ?>"></div>
            </div>
            ...
            <div class="fn fn-actions">
                ...
                ?>
                    <div class="act-row <?= $on ? '' : 'act-off' ?>">
                        ...
                        <span class="act-name" title="<?= htmlspecialchars($a['name']) ?>"><?= htmlspecialchars($a['name']) ?></span>
                    </div>
                ...
                <tr<?= $active ? '' : ' style="opacity:.35"' ?>>
                    ...
                    <td>
                        ...
                        <?php foreach ($sevs as $s):
                            $on = ($mask >> $s['bit']) & 1;
                        ?>
                        <div class="sev-c <?= $on ? $s['c'] : 'sev-off' ?>" title="<?= htmlspecialchars($s['t']) ?>"><?= $s['l'] ?></div>
                        <?php endforeach; ?>
                        </div>
                    </td>
                </tr>
        ...
        <div class="uo-hg">
            ...
            ?>
            <span class="hg-chip <?= $pi['cls'] ?>"
                  title="<?= htmlspecialchars($gname) ?> — <?= htmlspecialchars($pi['label']) ?>">
                <?= htmlspecialchars($gname) ?> <span class="p-lvl">[<?= $pi['label'] ?>]</span>
            </span>
            <?php endforeach; endif; ?>
        </div>
